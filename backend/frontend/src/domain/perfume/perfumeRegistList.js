...
function PerfumeRegistList() {
  ...
  return (
    <div className="perfumeRegistList">
      ...
      <div className="container mt-50 mb-100" style={{ width: "70%", minWidth: "500px" }}>
      <div>
              ...
              registList.map((data) => (
                <div className="row perfume">
                  ...
                  <div className="col-md-7 perfumeText d-flex flex-column justify-content-center align-items-start">
                    <h4>{data.name}</h4>
                    <h4>{data.company}</h4>
                    <textarea readOnly rows="3" className="describe">{data.describe}</textarea>
                    {/* <h5>향수에 대한 설명이</h5>
                    <h5>이렇게 두줄로 들어갑니다.</h5> */}
                    <h5>신청자 : {data.user}</h5>
                  </div>
                  ...
                  }
                  {
                    ...
                    <>
                    <div>
                      <button id={`${data.id}`} onClick={(e)=> setStatus(0,e)}>검토</button>
                      <button id={`${data.id}`} onClick={(e)=> setStatus(1,e)}>완료</button>
                      <button id={`${data.id}`} onClick={(e)=> setStatus(2,e)}>반려</button>
                    </div>
                    </>
                    :
                    <>
